...
def _packet(*, padding: bool) -> RTPPacket:
    ...
    return RTPPacket(header + b'\x00')


def test_padding_bit_parsed_from_header():
    assert _packet(padding=True).padding is True
    assert _packet(padding=False).padding is False
